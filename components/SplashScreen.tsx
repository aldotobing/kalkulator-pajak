...
export const SplashScreen: React.FC<Props> = ({ onFinish }) => {
  ...
  useEffect(() => {
    ...
    const exitTimer = setTimeout(() => setIsExiting(true), 2800); // Start exit anim
    const finishTimer = setTimeout(onFinish, 3400); // Unmount
  ...
  return (
    <div 
      className={`fixed inset-0 z-[9999] flex flex-col items-center justify-center bg-slate-900 overflow-hidden transition-all duration-700 ease-[cubic-bezier(0.7,0,0.3,1)] ${
        isExiting ? 'opacity-0 -translate-y-12 scale-105 pointer-events-none' : 'opacity-100'
      }`}
    >
      ...
      <div className="relative z-10 flex flex-col items-center">
        ...
        <div className="mt-12 w-48 h-1 bg-slate-800 rounded-full overflow-hidden relative">
           <div className="absolute inset-y-0 left-0 bg-gradient-to-r from-blue-500 to-cyan-500 w-full h-full rounded-full origin-left animate-[scaleIn_2.5s_ease-out_forwards]"></div>
        </div>
      </div>
